...
<tr>
    ...
    <td>
        <input type="number" name="{{ $name('quantity') }}" value="{{ $line['quantity'] ?? 1 }}" class="form-control form-control-sm text-end @if ($error('quantity')) is-invalid @endif" step="any" min="0" required aria-label="{{ __('Quantity') }}">
    </td>
    <td class="small text-body-secondary" data-unit>{{ $selectedProduct?->unit?->code }}</td>
    <td>
        <input type="number" name="{{ $name('unit_price') }}" value="{{ $line['unit_price'] ?? '' }}" class="form-control form-control-sm text-end @if ($error('unit_price')) is-invalid @endif" step="any" min="0" required data-fill="unit_price" aria-label="{{ __('Unit price') }}">
    </td>
    <td>
