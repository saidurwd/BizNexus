...
@section('title', __('Intercompany'))

@section('content_header')
    <h1>{{ __('Intercompany Transactions') }}</h1>
    <p class="text-muted mb-0">{{ __('Charges to another company of the group, booked in both ledgers with each company\'s own rates. Balances are eliminated in consolidation.') }}</p>
@endsection

@section('content')
    @can('finance.intercompany.create')
        <div class="card">
            <div class="card-header"><h3 class="card-title">{{ __('New charge from this company') }}</h3></div>
            <div class="card-body">
                @if ($counterparties->isEmpty())
                    <p class="mb-0 text-muted">{{ __('You need the intercompany permission in at least one other company of the group.') }}</p>
                @else
                    <form method="POST" action="{{ route('finance.intercompany.store') }}">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="target_company_id">{{ __('Charge to') }}</label>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label for="source_account_id">{{ __('Income account here') }}</label>
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label for="target_account_code">{{ __('Cost account code in the other company') }}</label>
                        ...
                        <button type="submit" class="btn btn-primary">{{ __('Post in both companies') }}</button>
                    </form>
                @endif
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-body table-responsive">
            <table class="table table-bordered table-striped">
                <thead><tr><th>{{ __('Date') }}</th><th>{{ __('From') }}</th><th>{{ __('To') }}</th><th>{{ __('Description') }}</th><th class="text-end">{{ __('Amount') }}</th></tr></thead>
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center text-muted">{{ __('No intercompany transactions.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
            {{ $transactions->links() }}
