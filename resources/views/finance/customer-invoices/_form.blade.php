    ...
    <div class="col-md-4">
        ...
        <select id="customer_id" name="customer_id" class="form-select @error('customer_id') is-invalid @enderror" data-searchable required>
            ...
            @foreach ($customers as $customer)
                <option value="{{ $customer->id }}" @selected((string) old('customer_id', $invoice?->customer_id) === (string) $customer->id)>{{ $customer->customer_code ? $customer->customer_code.' — ' : '' }}{{ $customer->name }}</option>
            @endforeach
        </select>
