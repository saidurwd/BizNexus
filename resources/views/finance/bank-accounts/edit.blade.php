...
@section('title', __('Edit Bank Account - BizNexus'))

@section('content_header')
    <h1>{{ __('Edit Bank Account') }}</h1>
@endsection
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('finance.bank-accounts.update', $bankAccount->id) }}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-control">
                                <option value="active" @selected(old('status', $bankAccount->status) == 'active')>{{ __('Active') }}</option>
                                <option value="inactive" @selected(old('status', $bankAccount->status) == 'inactive')>{{ __('Inactive') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    <button type="submit" class="btn btn-primary">{{ __('Update Bank Account') }}</button>
