...
<div class="row">
    @unless ($tax)
        ...
        <div class="col-md-3">
            <div class="mb-3">
                <label for="effective_from">{{ __('Effective from') }}</label>
                <input type="date" class="form-control" id="effective_from" name="effective_from" value="{{ old('effective_from') }}">
                <small class="form-text text-muted">{{ __('Leave empty to apply to all dates.') }}</small>
            </div>
        </div>
    @endunless
    <div class="col-md-3">
    ...
    <div class="col-md-3">
        <div class="mb-3">
            <label for="region_code">{{ __('Region / state code') }}</label>
        ...
        <div class="col-md-3">
            <div class="mb-3">
                ...
                <select class="form-control" id="{{ $field }}" name="{{ $field }}">
                    <option value="1" @selected($selectBool($field, $default) === '1')>{{ __('Yes') }}</option>
                    <option value="0" @selected($selectBool($field, $default) === '0')>{{ __('No') }}</option>
                </select>
            </div>
        </div>
    @endforeach
    <div class="col-md-3">
        <div class="mb-3">
            ...
            <select name="status" id="status" class="form-control">
                <option value="active" @selected(old('status', $tax?->status ?? 'active') === 'active')>{{ __('Active') }}</option>
                <option value="inactive" @selected(old('status', $tax?->status) === 'inactive')>{{ __('Inactive') }}</option>
            </select>
        </div>
    </div>
</div>
