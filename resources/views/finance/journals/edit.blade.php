...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('finance.journals.update', $journal->id) }}" method="POST" id="journalForm">
                @csrf
                @method('PUT')
                <input type="hidden" name="company_id" value="{{ $journal->company_id }}">
                ...
                <div class="mt-4">
                    ...
                    <table class="table table-bordered" id="journalLinesTable">
                        ...
                        <tbody id="linesBody">
                            @foreach(old('lines', $journal->lines->toArray()) as $index => $line)
                                <tr class="line-row">
                                    <td>
